...
public class ProductController {
    ...
    @ResponseBody
    public JSONResponse viewProduct(@PathVariable Long prodId, @ModelAttribute("userObj") User user) throws Exception {
        return new JSONResponse(true, productDataAccess.get(prodId));
    }
    
    @GetMapping(path = "/products")
    @ResponseBody
    public JSONResponse viewAllProduct(@ModelAttribute("userObj") User user) throws Exception {
        return new JSONResponse(true, productDataAccess.getAll());
    }
    
    @GetMapping(path = "/category")
    @ResponseBody
    public JSONResponse viewAllCatgeory(@ModelAttribute("userObj") User user) throws Exception {
        return new JSONResponse(true, productDataAccess.getAllCategory());
    }
    
    @PostMapping("/products")
    @ResponseBody
    public JSONResponse addProduct(@ModelAttribute Product product, BindingResult result, @RequestParam Long categoryId, @ModelAttribute("userObj") User user) throws Exception {
        if(result.hasErrors()) {
    ...
    @ResponseBody
    public JSONResponse addCategory(@ModelAttribute Category category, BindingResult result, @ModelAttribute("userObj") User user) throws Exception {
        if(result.hasErrors()) {
            throw new Exception("Binding Result error");
        }
